...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ZillionCoin: starting difficulty is 1 / 2^20
...
const string strMessageMagic = "ZillionCoin Signed Message:\n";
...
{
    const int nTargetSpacing = (pLastBlock->nHeight > (int)getFirstHardforkBlock())? 60 : 600; // ZillionCoin: 1 minute after block 2200
    const int nTargetTimespan = (pLastBlock->nHeight > (int)getSecondHardforkBlock())? 6*60*60 : 24*60*60; // ZillionCoin: 6 hours after second hardfork
...
{
    const char* pszTimestamp = "bitcoin at 4000 dollars and bitfinex kicked out all USA account holders";
    ...
    block.nVersion = 1;
    block.nTime    = 1502670700; //fTestNet? 1496033996 : 1495841000; //1406620001 : 1406620000;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey* preservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZillionCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZillionCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZillionCoinMiner(CWallet *pwallet)
{
    printf("ZillionCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zillioncoin-miner");
    ...
    {
        printf("ZillionCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZillionCoinMiner, pwallet));
}
