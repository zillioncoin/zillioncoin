...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown)
    {
        ...
        {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (zillioncoin.conf).");
            QMessageBox::warning(this, "ZillionCoin Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
